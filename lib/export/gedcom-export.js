 ...
 *
 * Queries all data for a given tree from the database and reconstructs
 * an EnrichedDocument JSON payload compatible with the Go library API's
 * /api/v1/export endpoint.
 *
 * This is the reverse of lib/import/gedcom-import.js.
 */

import { prisma } from '@/lib/database/prisma';

/**
 * Build an EnrichedDocument from database records for a given fileUuid.
 *
 * @param {string} fileUuid - The gedcom_files.id UUID
 * @returns {Promise<object>} EnrichedDocument-compatible JSON
 */
export async function buildEnrichedDocumentFromDB(fileUuid) {
  const where = { fileUuid };

  const [
    dates, places, surnames, givenNames, events,
    notes, sources, repositories, media,
    individuals, families,
    individualEvents, familyEvents,
    individualNotes, familyNotes, eventNotes, sourceNotes,
    individualSources, familySources, eventSources,
    individualSurnames, individualGivenNames, familySurnames,
    individualMedia, familyMedia, sourceMedia,
    sourceRepositories,
    parentChild, familyChildren, spouses,
  ] = await Promise.all([
    ...
    prisma.gedcomIndividual.findMany({ where, orderBy: { id: 'asc' } }),
    ...
    prisma.gedcomIndividualSurname.findMany({ where }),
    prisma.gedcomIndividualGivenName.findMany({ where }),
  ...
  // Build UUID → xref maps for individuals and families
  const indiIdToXref = {};
  for (const indi of individuals) indiIdToXref[indi.id] = indi.xref;
  const famIdToXref = {};
  for (const fam of families) famIdToXref[fam.id] = fam.xref;

  // Build UUID → xref for sources (used by event source reconstruction)
  const sourceIdToXref = {};
  for (const src of sources) sourceIdToXref[src.id] = src.xref;

  // Pre-build event → owner map for O(1) lookups
  const eventOwnerMap = {};
  ...
  return {
    ...
    // Junction tables - names
    IndividualSurnames: individualSurnames.map((s) => ({
      individual_xref: indiIdToXref[s.individualId] || '',
      surname_index: surnameIndex[s.surnameId] ?? -1,
      name_type: s.nameType || 'birth',
      is_primary: s.isPrimary || false,
    })),

    IndividualGivenNames: individualGivenNames.map((g) => ({
      individual_xref: indiIdToXref[g.individualId] || '',
      given_name_index: givenNameIndex[g.givenNameId] ?? -1,
