...
export async function buildEnrichedDocumentFromDB(fileUuid) {
  ...
  const [
    ...
    individualNameForms, familySurnames,
    individualMedia, familyMedia, sourceMedia, eventMedia,
    sourceRepositories,
  ...
  return {
    Individuals: individuals.map((indi) => {
      const occValues = (indi.individualOccupations || []).map(
        (io) => io.valueOverride || io.occupation?.value || ''
      ).filter(Boolean);
      const natValues = (indi.individualNationalities || []).map(
        (in_) => in_.nationality?.value || ''
      ).filter(Boolean);
      const occupationValues = occValues.length > 0
        ? occValues
        : (indi.occupation ? [indi.occupation] : []);
      const nationalityValues = natValues.length > 0
        ? natValues
        : (indi.nationality ? [indi.nationality] : []);
      return {
        id: indi.id,
        xref: indi.xref,
    ...
    // Junction tables - names (NameForm model)
