import { storyDocumentToDbPayload } from '@ligneous/story-creator';

async function resolveTagIds(tx, userId, names) {
  const ids = [];
  for (const raw of names) {
    ...
    const created = await tx.tag.create({
      data: { userId, name, isGlobal: false, createdBy: userId },
      select: { id: true },
    });
    ids.push(created.id);
  }
...
/**
 * Within an interactive transaction: replace story metadata, chapter/section tree,
 * and junction rows from a full StoryDocument.
 */
export async function replaceStoryFromDocumentInTx(tx, { storyId, userId, doc }) {
  if (doc.id !== storyId) throw new Error('Document id must match story route id');
  ...
  for (const ch of payload.chapters) {
    const createdCh = await tx.storyChapter.create({
      data: { storyId, title: ch.title, sortOrder: ch.sortOrder, slug: ch.slug },
    });
  ...
  if (individualIds.length > 0) {
    await tx.storyIndividual.createMany({
      data: individualIds.map((individualId, sortOrder) => ({ storyId, individualId, sortOrder })),
    });
  }
  if (familyIds.length > 0) {
    await tx.storyFamily.createMany({
      data: familyIds.map((familyId, sortOrder) => ({ storyId, familyId, sortOrder })),
    });
  }
  if (eventIds.length > 0) {
    await tx.storyEvent.createMany({
      data: eventIds.map((eventId, sortOrder) => ({ storyId, eventId, sortOrder })),
    });
  }
  if (placeIds.length > 0) {
    await tx.storyPlace.createMany({
      data: placeIds.map((placeId, sortOrder) => ({ storyId, placeId, sortOrder })),
    });
  }
  ...
  if (tagIds.length > 0) {
    await tx.storyTag.createMany({
      data: tagIds.map((tagId) => ({ storyId, tagId, taggedBy: userId })),
    });
  }

  if (albumIds.length > 0) {
    await tx.albumStory.createMany({
      data: albumIds.map((albumId, sortOrder) => ({ storyId, albumId, sortOrder, addedBy: userId })),
    });
  }

  const row = await tx.story.findUniqueOrThrow({ where: { id: storyId }, select: { updatedAt: true } });
