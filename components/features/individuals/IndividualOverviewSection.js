...
export default function IndividualOverviewSection({ individual, treeId }) {
  ...
  const givenNames = (individual.individualGivenNames || [])
    .map((ig) => ig.givenName)
    .filter(Boolean);
  const surnames = (individual.individualSurnames || [])
    .map((is) => is.surname)
    .filter(Boolean);
        ...
        <div className="card-body p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-base-content/10">
            <div className="flex items-center gap-2 text-sm">
              <Tag size={16} className="text-base-content/50" />
            ...
            {individual.deathDateDisplay && (
              <div className="flex items-center gap-2 text-sm">
                <CalendarX size={16} className="text-base-content/50" />
