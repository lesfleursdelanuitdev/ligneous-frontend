...
export default function TreeDatesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await authFetch(`/api/trees/${treeId}/dates?limit=500`);
  ...
  const headers = [{ label: 'Date', key: 'value', sortable: false }, { label: 'Count', key: 'count', sortable: false }];
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-base-content">Dates</h1>
            <p className="text-base-content/60 mt-1">{items.length} date entries</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="card bg-base-100 border border-base-content/10 p-12 text-center rounded-box"><p className="text-base-content/60">No dates in this tree.</p></div>
        ) : (
          <DataView
            view={view}
            items={items}
            renderCard={(row) => (
              <div className="card bg-base-200 rounded-box p-4">
                <div className="font-medium">{row.value ?? row.date ?? row.normalized}</div>
                <div className="text-sm text-base-content/70">Count: {row.count ?? row.events_count ?? 0}</div>
              </div>
            )}
            renderRow={(row) => (
              <>
                <td className="px-6 py-4">{row.value ?? row.date ?? row.normalized}</td>
                <td className="px-6 py-4">{row.count ?? row.events_count ?? 0}</td>
              </>
            )}
