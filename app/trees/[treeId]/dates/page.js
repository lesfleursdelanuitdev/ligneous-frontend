...
import { GitMerge, BarChart2, BarChart3, CalendarDays, ListFilter } from 'lucide-react';
  ...
  return (
    <DashboardMainContentLayout treeId={treeId} title="Dates" subtitle={`${totalItems} date entries`}>
        <DataViewContainer
          ...
          defaultView="list"
          renderCard={(row) => {
            const dateLabel = row.original || '\u2014';
            const eventsUrl = `/trees/${treeId}/events?date_id=${encodeURIComponent(row.id)}${dateLabel !== '\u2014' ? `&date=${encodeURIComponent(dateLabel)}` : ''}`;
            return (
              <Link href={eventsUrl} className="block h-full">
                <BaseCard hoverable className="h-full transition-colors hover:bg-base-200/50">
                  <div className="space-y-3">
                    <div className="font-medium text-base-content">{dateLabel}</div>
          ...
          renderRow={(row) => {
            ...
            <>
              <td className="px-6 py-4">
                <Link href={eventsUrl} className="link link-primary font-medium">{dateLabel}</Link>
              </td>
          ...
          actions={[
            { key: 'view', label: 'View events', icon: ListFilter, href: (item) => `/trees/${treeId}/events?date_id=${encodeURIComponent(item.id)}${item.original ? `&date=${encodeURIComponent(item.original)}` : ''}` },
            { key: 'dateDetail', label: 'Date details', icon: CalendarDays, href: (item) => `/trees/${treeId}/dates/${item.id}` },
